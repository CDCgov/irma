...
foreach $rn ( keys(%pairs) ) {
			...
			for($i=$start;$i<=$end;$i++) {
				...
				if ( defined($insByIndex{$rn}{$K1}{$i}) && defined($insByIndex{$rn}{$K2}{$i}) ) {
					...
					if ( $ins1 eq $ins2 ) {
						...
						$insTable{$rn}{$i}{$ins1}[0]++;
						$qIntSum = 0;
						for $qIntVal ( unpack("c* i*",$qSeqNew) ) { $qIntSum += $qIntVal-33; }
						$insTable{$rn}{$i}{$ins2}[1] += $qIntSum/length($ins1);
					} elsif ( $ins2 =~ /$ins1/ ) {
						# 1 in 2
						$mSeq .= $ins1;
						...
						$insTable{$rn}{$i}{$ins1}[0]++;
						$qIntSum = 0;
						for $qIntVal ( unpack("c* i*",$insByIndex{$rn}{$K1}{$i}[1]) ) { $qIntSum += $qIntVal-33; }
						$insTable{$rn}{$i}{$ins1}[1] += $qIntSum/length($ins1);
					} elsif ( $ins1 =~ /$ins2/ ) {
						# 2 in 1
						$mSeq .= $ins2;
						...
						$insTable{$rn}{$i}{$ins2}[0]++;
						$qIntSum = 0;
						for $qIntVal ( unpack("c* i*",$insByIndex{$rn}{$K2}{$i}[1]) ) { $qIntSum += $qIntVal-33; }
						$insTable{$rn}{$i}{$ins2}[1] += $qIntSum/length($ins2);
					} else {
						$insErr++;
					}
	...
	if ( $insObs > 0 ) {
		...
		$insErr /= $insObs;
		$IEbyRef{$rn} = $insErr;
		print OUT $rn,"\tMinimumInsertionErrorRate\t$insErr\n";
	} else {
		print OUT $rn,"\tMinimumInsertionErrorRate\t0\n";
	}
}
close(SAM);
close(OUT);
...
foreach $rn ( keys(%tableByRname) ) {
	...
	for($p=0;$p<$N;$p++) {
		...
		if ( defined($tableByRname{$rn}[$p]{'-'}) ) {
			print COVG ($p+1),"\t",($total-$tableByRname{$rn}[$p]{'-'}),"\t",$consensus,"\t",$tableByRname{$rn}[$p]{'-'};
		} else {
		...
		if ( ! defined($tableByRname{$rn}[$p]{'N'}) ) {
			...
			print COVG "\t",$tableByRname{$rn}[$p]{'N'},"\n";
		}
		print CONS $consensus;
		$consensuses{$rn} .= $consensus;
			...
			if ( $base eq $consensus ) {
				...
				if ( $base ne 'N' ) {	
					...
					if ( !($noGap && $base eq '-') && $freq >= $minFreq && $count >= $minCount && $quality >= $minQuality  && $total >= $minTotal ) {
						...
						if ( $freq <= $EE ) { $hFreq = $freq; }
						if ( $confidence < $minConf || $freq <= $pairedUB || $freq <= $qualityUB ) { next; }

						$variants{$rn}{$p}{$base} = $freq;

						$varLine{$rn}{$p}{$base} = $rn."\t".($p+1)."\t".$total."\t";
						$varLine{$rn}{$p}{$base} .= $consensus."\t".$base."\t".$conCount."\t".$count."\t";
						$varLine{$rn}{$p}{$base} .= $conFreq."\t".$freq."\t".$conQuality."\t".$quality."\t";
						$varLine{$rn}{$p}{$base} .= $confidence."\t".$pairedUB."\t".$qualityUB."\n";
					# any variant
...
close(CONS);
close(COVG);
foreach $rn ( keys(%varLine) ) {
	foreach $p ( sort { $a <=> $b } keys(%{$varLine{$rn}}) ) {
		foreach $base ( sort { $varLine{$rn}{$p}{$a} cmp $varLine{$rn}{$p}{$b} } keys(%{$varLine{$rn}{$p}}) ) {
			if ( $autoFreq ) { 
				if ( $variants{$rn}{$p}{$base} > $hFreq ) {
					print VARS $varLine{$rn}{$p}{$base};
				} else {
					delete($variants{$rn}{$p}{$base});	
				}
			} else {
				print VARS $varLine{$rn}{$p}{$base};
			}
...
open(INSV,'>',$ARGV[2].'-insertions.txt') or die("ERROR: cannot open $ARGV[2]-insertions.txt for writing.\n");
...
foreach $rn ( sort { $a <=> $b } keys(%insTable) ) {
	$N = $lenByRname{$rn};
	@sequences = @{$alignmentByRname{$rn}};

	foreach $p ( sort { $a <=> $b } keys(%{$insTable{$rn}}) ) {
		foreach $insert ( keys(%{$insTable{$rn}{$p}}) ) {
			$called = "TRUE";
			$count = $insTable{$rn}{$p}{$insert}[0];
			if ( $count > 0 ) {
				$quality = $insTable{$rn}{$p}{$insert}[1] / $count;
			} else {
				$quality = 0;
			}

			if ( $count < $minCount || $quality < $minQuality ) { $called = "FALSE"; }

			# VALID if not able to hang: 0 < $p < ($N-1)
			$total = 0;
			$pp=$p+1;
			foreach $theSeq ( @sequences ) {
			...
			if ( $freq < $minFreq || $total < $minTotal ) { $called = "FALSE"; }

			$EE = 1/(10**($quality/10));
			$confidence = calcProb($freq,$EE);

			if ( $total == 0 ) {
				print STDERR "WARNING (mergeCallPhase): $rn/$p insert has 0 s1 + s2 total but should have >= $count.\n";
				$pairedUB = UB($IEbyRef{$rn},$count);
				$qualityUB = UB($EE,$count);
			} else {
				$pairedUB = UB($IEbyRef{$rn},$total);
				$qualityUB = UB($EE,$total);
			}
			...
			$left = $right = '';
			if ( $p < 5 ) {
				$left = substr($consensuses{$rn},0,$p+1);
			} else {
				$left = substr($consensuses{$rn},$p-4,5);
			}

			if ( $p > ($N-6) ) {
				$right = substr($consensuses{$rn},$p+1,$N-$p-1);
			} else {
				$right = substr($consensuses{$rn},$p+1,5);
			}
...
if ( scalar(@varKeys) > 0 ) {
	...
	foreach $ref ( @varKeys ) {
		...
		@vars = sort { $a <=> $b } keys(%{$variants{$ref}});
		%readPats = ();
