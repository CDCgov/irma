...
#
# Description: aggegrate rough alignment statistic files for iterative consensus generation
...
if ($notSkipExtension) {
    ...
    # LEADER
    $leader    = '';
    $threshold = max( $minPadCount, int( $max5 / $denom ) + 1 );
    ...
    # TRAILER
    $trailer   = '';
    $threshold = max( $minPadCount, int( $max3 / $denom ) + 1 );
