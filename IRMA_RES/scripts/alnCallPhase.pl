...
if ( defined($sigLevel) ) {
	...
	sub UB($$) {
		...
		if ( $V+($gamma1*$V+$gamma2) < 0 ) {
			print STDERR "Unexpected estimate for ($p,$N,$kappa)\n";
			return 0;
		}
	...
	if ( $REF_NAME eq $rn ) {
		...
		while($cigar =~ /(\d+)([MIDNSHP])/g ) {
			...
			if ( $op eq 'M' ) {
				...
				$InsCtable{$rpos}{$insert}++;
				$InsQtable{$rpos}{$insert} += ( sum(\@Qint,$qpos,$inc) / length($insert) ) - 33;
				$qpos += $inc;
			} elsif( $op eq 'S' ) {
				$qpos += $inc;
			} elsif( $op eq 'N' ) {
		...
		if ( $base eq $consensus ) {
			if ( $printAllAlleles ) {
				...
				if ( $base eq 'N' ) {
					...
					$quality = $conQuality;
					$pairedUB = 1; # UB($PE,$conCount);
					$qualityUB = 1;# UB($ee,$conCount);
					$total = $conCount;
				} elsif ( $base eq '-' ) {
					$quality = 'NA';
					$confidence = 'NA';
...
open(INSV,'>',$prefix.'-insertions.txt') or die("ERROR: cannot open $prefix-insertions.txt for writing.\n");
	...
	print INSV "\tExpectedErrorUB\tQualityEstimatedErrorUB\n";
foreach $p ( sort { $a <=> $b } keys(%InsCtable) ) {
	foreach $insert ( keys(%{$InsCtable{$p}}) ) {
		$called = "TRUE";
		$count = $InsCtable{$p}{$insert};
		if ( $count > 0 ) {
			$quality = $InsQtable{$p}{$insert} / $count;
		} else {
			$quality = 0;
		}
		...
		$confidence = calcProb($freq,$EE);
		$qualityUB = UB($EE,$total);
		$expectedUB = UB($IE,$total);

		if ( $confidence < $minConf || $freq <= $qualityUB ) { $called = "FALSE"; }
		...
		print INSV "\t",$confidence,"\t",$expectedUB,"\t",$qualityUB,"\t","\n";
	}
}
close(INSV);
