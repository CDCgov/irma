#!/usr/bin/env perl
# amplicon_dropout_pad
#
# Sam Shepard with Kristine Lacek - 2020
#
# Description: use the first round assembly to detect and pad amplicon dropout

use strict;
use warnings;

my ( $expected_length, $trim_ends, $message, $remove_inserted_N, $remove_deletions, $pad_adjacent, $coverage_in,
     $coverage_out );
    ...
    foreach my $idx ( 0 .. $#coverage ) {
        ...
        if ( substr( $padded_reference, $idx, 1 ) eq '*' ) {
            if ( $fields[$iCon] eq '-' ) {
                ...
                print STDERR
                  "Expected '-' in table at $fields[$iPos], but found $fields[$iCon] instead, printing existing line.\n";
                print COV $line, "\n";
            }
        } else {
            print COV $line, "\n";
