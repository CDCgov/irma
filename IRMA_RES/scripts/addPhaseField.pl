...
# addPhasefield
#
# Samuel S. Shepard - 2017
#
# Description: add to variant table the computed phase field
...
if ( scalar(@variants) < 3 ) {
    ...
    %phaseCount = %phaseByAllele = %rankByPhase = ();
    $i          = 0;
    ...
    if ( $aIndex == -1 || $pIndex == -1 ) {
        die("No position or minority allele field found in the variant table header.\n");
    }
