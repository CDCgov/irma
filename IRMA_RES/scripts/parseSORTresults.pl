...
#
# Description: parse the results for sorting reads (into genomes or gene segments) and apply read count / pattern limits.

use Getopt::Long;
GetOptions(
            'pattern-list|P=s'      => \$patternList,
            'ignore-annotations|G'  => \$ignoreAnnotations,
            'min-read-count|C=i'    => \$minimumRcount,
            'min-read-patterns|D=i' => \$minimumRPcount,
