...
# Sam Shepard - 2017
#
# Description: parse BLAT output for sorting, classification and rough alignment

use File::Basename;
use Getopt::Long;
use Storable;
GetOptions(
            'separate-ha-na-og|T'  => \$triplet,
            'classify|C'           => \$classify,
...
if ($classify) {
    ...
    foreach $sg ( keys(%supergroups) ) {
        open( $supergroupHandle{$sg}, '>', "$path/$name.match.$sg" )
          or die("Cannot open $path/$name.match.$sg for writing.\n");
    }
}
        ...
        if ( $sortOutChim && index( $Q{$header}, 'c' ) != -1 ) {
            ...
            foreach $gene ( keys( %{ $maxGene{$q} } ) ) {
                ...
                if ( defined($alignSequences) ) {
                    recordStats( \%alignStats, $gene, alignedBLAT( $matchLine, $seq2, $gene ) );
                }
