...
#
# Description: generates a custom VCF files that understands IRMA's variant calling output
...
if ($takeSig) {
    print '##INFO=<ID=QUB,Number=A,Type=Float,Description="Second-order corrected ', $sigLevel,
      '% binomial confidence upper bound on quality error estimate">', "\n";
    print '##INFO=<ID=PUB,Number=.,Type=Float,Description="Second-order corrected ', $sigLevel,
      '% binomial confidence upper bound on paired error estimate">', "\n";
}
...
if ( $minConf > 0 ) {
    print sprintf( '##FILTER=<ID=conf%0f,Description="Minimum Confidence (Proportion not Machine Error) %0f">',
                   $minConf, $minConf ),
      "\n";
}

if ($takeSig) {
    print '##FILTER=<ID=sigP', $sigLevel, ',Description="Second-order corrected ', $sigLevel,
      '% binomial confidence upper bound on paired error estimate">', "\n";
    print '##FILTER=<ID=sigQ', $sigLevel, ',Description="Second-order corrected ', $sigLevel,
      '% binomial confidence upper bound on quality error estimate">', "\n";
}
...
while ( $line = <IN> ) {
    ...
    (
       $Reference_Name, $Position,  $Allele,          $Count,
       $Total,          $Frequency, $Average_Quality, $ConfidenceNotMacErr,
       $PairedUB,       $QualityUB, $Allele_Type
    ) = @fields;
...
if ( defined($autoFreq) ) {
    if ( $heurFreq > $minFreq ) {
        $minFreq = $heurFreq;
        print sprintf( '##FILTER=<ID=fsnv%0f,Description="(AUTO) Minority Allele Frequency Lower Bound %0f">',
                       $minFreq, $minFreq ),
          "\n";
    } else {
...
# Reference_Name Upstream_Position Insert Context Called Count Total Frequency Average_Quality ConfidenceNotMacErr PairedUB QualityUB
    (
       $Reference_Name,  $Upstream_Position,   $Insert,   $Context,
       $Called,          $Count,               $Total,    $Frequency,
       $Average_Quality, $ConfidenceNotMacErr, $PairedUB, $QualityUB
    ) = @fields;
...
foreach $p ( sort { $a <=> $b } keys(%minorityTable) ) {
    ...
    foreach $b ( keys( %{ $minorityTable{$p} } ) ) {
        ...
        if ( defined($rewriteVariants) ) {
            (
               $cReference_Name, $cPosition,  $cAllele,          $cCount,
               $cTotal,          $cFrequency, $cAverage_Quality, $cConfidenceNotMacErr,
               $cPairedUB,       $cQualityUB, $cAllele_Type
            ) = @{ $majorityTable{$p} };
