...
# Samuel Shepard - 2021
#
# Description: use variant calling information to amend the plurality consensus
...
my (
     $minCount,            $minTotal,           $minFreq,          $name,             $convertSeg,
     $prefix,              $faHeaderSuffix,     $minFreqDel,       $minFreqIns,       $delFile,
     $insFile,             $covgRewrite,        $replaceNotEncode, $replaceWithPhase, $minConsensusSupport,
     $minConsensusQuality, $outputCoverageFile, $printFinalName,   $a2mReference
);
...
foreach my $line (@variants) {
    my (
         $ref,  $pos,   $total, $majAllele, $allele,   $majCount,  $count, $majFreq,
         $freq, $majAQ, $aq,    $con,       $pairedUB, $qualityUB, $phase
    ) = split( "\t", $line );
    $allele = uc($allele);
...
if ( defined($delFile) ) {
    ...
    while ( my $line = <IN> ) {
        chomp($line);
        my ( $Reference_Name, $Upstream_Position, $Length, $Context, $Called, $Count, $Total, $Frequency, $PairedUB ) =
          split( "\t", $line );
